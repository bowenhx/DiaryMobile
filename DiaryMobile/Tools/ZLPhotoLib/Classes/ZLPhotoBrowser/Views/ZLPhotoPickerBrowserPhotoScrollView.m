...
- (void)longGesture:(UILongPressGestureRecognizer *)gesture{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        if (!self.sheet) {
            self.sheet = [[UIActionSheet alloc] initWithTitle:@"提示" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"保存到相冊" otherButtonTitles:nil, nil];
        }
    ...
    if (buttonIndex == 0){
        if([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeSavedPhotosAlbum]) {
            ...
            if (_photoImageView.image) {
                [self showMessageWithText:@"沒有用戶權限,保存失敗"];
            }
        }
    }
}
    ...
    if (photo.photoURL.absoluteString.length) {
        ...
        if (photoRange.location != NSNotFound){
            ...
            if ([photo.photoURL.absoluteString hasSuffix:@"gif"]) {
                [_photoImageView getImage:photo.photoURL.absoluteString result:^(NSData *data) {
                    self.isLoadingDone = YES;
                    if (data) {
                        [self setProgress:1.0];
                        photo.photoImage = [UIImage imageWithData:data];
                        _photoImageView.image = photo.photoImage;
                        [weakSelf displayImage];
                        
                        _photoImageView.gifData = data;
                        [_photoImageView startGIF];
