...
- (UIButton *)deleleBtn{
    if (!_deleleBtn) {
        ...
        if (_editText) {
            [deleleBtn setTitle:_editText forState:UIControlStateNormal];
        }else{
        ...
        //设定右上角"删除"/"保存"按钮的约束.适配ipX
        CGFloat deleteButtonWidth = 50;
        CGFloat deleteButtonTop = kNAV_BAR_HEIGHT - 54;
        CGFloat deleteButtonRight = -10;
        [deleleBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.width.height.mas_equalTo(deleteButtonWidth);
            make.top.equalTo(self.view).offset(deleteButtonTop);
            make.right.equalTo(self.view).offset(deleteButtonRight);
        }];
    }
    return _deleleBtn;
}
...
-(void)setPageLabelPage:(NSInteger)page{
    self.pageLabel.text = [NSString stringWithFormat:@"%zd / %zd",page + 1, self.photos.count];
    if (self.isPush) {
...
- (void)delete{
    //准备保存
    if ([_editText isEqualToString:@"保存"]) {
        ...
        if (cell) {
            if([[[cell.contentView subviews] lastObject] isKindOfClass:[UIView class]]){
                ZLPhotoPickerBrowserPhotoScrollView *scrollView = [[[[cell.contentView subviews] lastObject] subviews] lastObject];
                UIImage *image = scrollView.photoImageView.image;
                if (image) {
                    //保存图片到本地相册
                    ...
                    [self.view showHUDTitleView:@"請加載完圖片后再保存" image:nil];
                }
            }
        }
        return;
    ...
    [[[UIAlertView alloc] initWithTitle:@"確定要刪除此圖片"
                                message:nil
                               delegate:self
