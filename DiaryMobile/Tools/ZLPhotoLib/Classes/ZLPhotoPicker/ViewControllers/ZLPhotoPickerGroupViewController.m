...
- (void) jump2StatusVc{
    ...
    for (ZLPhotoPickerGroup *group in self.groups) {
        if (self.status == PickerViewShowStatusCameraRoll && ([group.groupName isEqualToString:@"Camera Roll"] || [group.groupName isEqualToString:@"機膠捲"])) {
            gp = group;
            break;
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    [[NSNotificationCenter defaultCenter] postNotificationName:@"kSelectImageNotificatiion" object:nil];
