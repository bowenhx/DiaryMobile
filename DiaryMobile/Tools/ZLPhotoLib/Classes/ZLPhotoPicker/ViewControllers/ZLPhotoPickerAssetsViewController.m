...
//    self.doneBtn.enabled = (count > 0);
    NSInteger macCount = self.maxCount ? self.maxCount : 9;
    [self.navigationItem.rightBarButtonItem setTitle:[NSString stringWithFormat:@"完成%ld/%ld",(long)count,(long)macCount]];
...
- (void)viewDidLoad {
    ...
    // 初始化底部ToorBar
    //[self setupToorBar]; (这里不需要显示底部图片查看)
...
//        weakSelf.doneBtn.enabled = (count > 0);
        [self.navigationItem.rightBarButtonItem setTitle:[NSString stringWithFormat:@"完成%ld/9",(long)count]];
        self.navigationItem.rightBarButtonItem.enabled = (count >0);
...
//            self.makeView.text = [NSString stringWithFormat:@"%ld",(unsigned long)self.selectAssets.count];
    ...
    [self.navigationItem.rightBarButtonItem setTitle:[NSString stringWithFormat:@"完成%ld/%ld",(long)self.selectAssets.count,(long)self.maxCount]];
